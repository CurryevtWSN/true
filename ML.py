...
HSD = st.sidebar.selectbox('High-salt diet',('No','Yes'),index=0)
PS = st.sidebar.selectbox('Poor sleep quality',('No','Yes'),index=0)
...
SmA = st.sidebar.slider("Smoking amount(package)", 0, 5, value=3, step=1)
SnT = st.sidebar.slider("Course of snoring(year)", 0, 50, value=30, step=1)
SuT = st.sidebar.slider("Course of choking(year)", 0, 30, value=15, step=1)
ESS = st.sidebar.slider("Epworth sleepiness scale(point)", 0, 25, value=10, step=1)
# SBS = st.sidebar.slider("SBS(单位)", 0, 10, value=5, step=1)
AHI = st.sidebar.slider("AHI(events/h)", 5.0, 150.0, value=50.0, step=0.1)
OAL = st.sidebar.slider("OAI(events/h)", 0.0, 130.0, value=50.0, step=0.1)
